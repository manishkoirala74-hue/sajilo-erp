-- 069_comprehensive_inventory_fix.sql
-- Fixes the update_current_stock trigger (TOCTOU race condition) and reconciles all historical data.
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        ...
        -- STEP 1: The "Seed"
        -- Safely initialize the row with 0 if it does not exist. 
        ...
        -- STEP 2: The "Update"
        -- Perform the actual math. Postgres will natively evaluate this against the CHECK constraint.
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        -- Handle item/godown reassignment gracefully
...
-- 2. Insert missing PurchaseInvoice ledger entries
...
-- 3. Insert missing SalesInvoice ledger entries
-- (Now that the trigger is fixed, this will succeed natively)
INSERT INTO public."InventoryLedger" (
    company_id, item_id, transaction_type, godown_id, quantity_in, quantity_out, 
    transaction_date, reference_id, reference_type, ledger_status
)
...
-- 4. Clean up any invalid or negative historical data by synchronizing CurrentStock with the Ledger
-- First, reset everything
UPDATE public."CurrentStock" SET current_qty = 0;

-- Then, aggregate and update
...
SET current_qty = GREATEST(agg.total_qty, 0) -- Forcing non-negative in case of deeply historical bad data
FROM AggregatedStock agg
WHERE cs.company_id = agg.company_id AND cs.godown_id = agg.godown_id AND cs.item_id = agg.item_id;

-- Ensure no items were missed
INSERT INTO public."CurrentStock" (company_id, godown_id, item_id, current_qty)
...
-- 5. Recalculate Item quantity_on_hand cache based on validated CurrentStock
