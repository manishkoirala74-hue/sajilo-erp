-- hotfix_postgres_uuid_error.sql
-- Fixes type mismatch error 'operator does not exist: uuid = text' by correctly typing parameters as UUID.

-- 1. Item Trading History RPC
DROP FUNCTION IF EXISTS get_item_recent_trading_history_rpc(TEXT);
DROP FUNCTION IF EXISTS get_item_recent_trading_history_rpc(TEXT, INT);

CREATE OR REPLACE FUNCTION get_item_recent_trading_history_rpc(p_item_id UUID)
RETURNS TABLE (
...
-- 2. Partner Ledger History RPC
DROP FUNCTION IF EXISTS get_partner_ledger_history_rpc(TEXT);
DROP FUNCTION IF EXISTS get_partner_ledger_history_rpc(TEXT, INT);

CREATE OR REPLACE FUNCTION get_partner_ledger_history_rpc(p_entity_id UUID)
RETURNS TABLE (
    journal_id TEXT,
