...
ALTER TABLE "GeneralLedgerLine"
  ALTER COLUMN journal_id TYPE UUID USING NULLIF(TRIM(journal_id::text), '')::UUID,
  ALTER COLUMN account_id TYPE UUID USING NULLIF(TRIM(account_id::text), '')::UUID,
  ALTER COLUMN entity_id TYPE UUID USING NULLIF(TRIM(entity_id::text), '')::UUID;

ALTER TABLE "GeneralLedgerJournal"
  ALTER COLUMN source_document_id TYPE UUID USING NULLIF(TRIM(source_document_id::text), '')::UUID;

ALTER TABLE "ChartOfAccount"
  ALTER COLUMN parent_account_id TYPE UUID USING NULLIF(TRIM(parent_account_id::text), '')::UUID;

ALTER TABLE "BusinessPartner"
  ALTER COLUMN receivable_account_id TYPE UUID USING NULLIF(TRIM(receivable_account_id::text), '')::UUID,
  ALTER COLUMN payable_account_id TYPE UUID USING NULLIF(TRIM(payable_account_id::text), '')::UUID;

ALTER TABLE "BankAccount"
  ALTER COLUMN gl_account_id TYPE UUID USING NULLIF(TRIM(gl_account_id::text), '')::UUID,
  ALTER COLUMN ledger_group_id TYPE UUID USING NULLIF(TRIM(ledger_group_id::text), '')::UUID;

ALTER TABLE "CompanySettings"
  ALTER COLUMN dep_factory_expense_account_id TYPE UUID USING NULLIF(TRIM(dep_factory_expense_account_id::text), '')::UUID,
  ALTER COLUMN dep_admin_expense_account_id TYPE UUID USING NULLIF(TRIM(dep_admin_expense_account_id::text), '')::UUID,
  ALTER COLUMN dep_accumulated_machinery_account_id TYPE UUID USING NULLIF(TRIM(dep_accumulated_machinery_account_id::text), '')::UUID,
  ALTER COLUMN dep_accumulated_office_account_id TYPE UUID USING NULLIF(TRIM(dep_accumulated_office_account_id::text), '')::UUID,
  ALTER COLUMN dep_accumulated_vehicle_account_id TYPE UUID USING NULLIF(TRIM(dep_accumulated_vehicle_account_id::text), '')::UUID,
  ALTER COLUMN gl_cash_account_id TYPE UUID USING NULLIF(TRIM(gl_cash_account_id::text), '')::UUID,
  ALTER COLUMN gl_bank_account_id TYPE UUID USING NULLIF(TRIM(gl_bank_account_id::text), '')::UUID,
  ALTER COLUMN gl_accounts_receivable_id TYPE UUID USING NULLIF(TRIM(gl_accounts_receivable_id::text), '')::UUID,
  ALTER COLUMN gl_accounts_payable_id TYPE UUID USING NULLIF(TRIM(gl_accounts_payable_id::text), '')::UUID,
  ALTER COLUMN gl_vat_payable_id TYPE UUID USING NULLIF(TRIM(gl_vat_payable_id::text), '')::UUID,
  ALTER COLUMN gl_sales_return_account_id TYPE UUID USING NULLIF(TRIM(gl_sales_return_account_id::text), '')::UUID,
  ALTER COLUMN gl_purchase_return_account_id TYPE UUID USING NULLIF(TRIM(gl_purchase_return_account_id::text), '')::UUID,
  ALTER COLUMN gl_default_sales_account_id TYPE UUID USING NULLIF(TRIM(gl_default_sales_account_id::text), '')::UUID,
  ALTER COLUMN gl_default_cogs_account_id TYPE UUID USING NULLIF(TRIM(gl_default_cogs_account_id::text), '')::UUID,
  ALTER COLUMN gl_default_inventory_account_id TYPE UUID USING NULLIF(TRIM(gl_default_inventory_account_id::text), '')::UUID,
  ALTER COLUMN gl_stock_variance_account_id TYPE UUID USING NULLIF(TRIM(gl_stock_variance_account_id::text), '')::UUID,
  ALTER COLUMN gl_opening_equity_account_id TYPE UUID USING NULLIF(TRIM(gl_opening_equity_account_id::text), '')::UUID,
  ALTER COLUMN gl_customer_ledger_group_id TYPE UUID USING NULLIF(TRIM(gl_customer_ledger_group_id::text), '')::UUID,
  ALTER COLUMN gl_supplier_ledger_group_id TYPE UUID USING NULLIF(TRIM(gl_supplier_ledger_group_id::text), '')::UUID,
  ALTER COLUMN gl_dual_ledger_group_id TYPE UUID USING NULLIF(TRIM(gl_dual_ledger_group_id::text), '')::UUID;

ALTER TABLE "UserCompany"
  ALTER COLUMN user_id TYPE UUID USING NULLIF(TRIM(user_id::text), '')::UUID,
  ALTER COLUMN company_id TYPE UUID USING NULLIF(TRIM(company_id::text), '')::UUID;

ALTER TABLE "Item"
  ALTER COLUMN category_id TYPE UUID USING NULLIF(TRIM(category_id::text), '')::UUID,
  ALTER COLUMN purchase_account_id TYPE UUID USING NULLIF(TRIM(purchase_account_id::text), '')::UUID,
  ALTER COLUMN sales_account_id TYPE UUID USING NULLIF(TRIM(sales_account_id::text), '')::UUID,
  ALTER COLUMN inventory_account_id TYPE UUID USING NULLIF(TRIM(inventory_account_id::text), '')::UUID,
  ALTER COLUMN discount_scheme_id TYPE UUID USING NULLIF(TRIM(discount_scheme_id::text), '')::UUID;

ALTER TABLE "ItemCategory"
  ALTER COLUMN parent_category_id TYPE UUID USING NULLIF(TRIM(parent_category_id::text), '')::UUID,
  ALTER COLUMN purchase_account_id TYPE UUID USING NULLIF(TRIM(purchase_account_id::text), '')::UUID,
  ALTER COLUMN sales_account_id TYPE UUID USING NULLIF(TRIM(sales_account_id::text), '')::UUID,
  ALTER COLUMN discount_scheme_id TYPE UUID USING NULLIF(TRIM(discount_scheme_id::text), '')::UUID;

ALTER TABLE "DiscountScheme"
  ALTER COLUMN item_id TYPE UUID USING NULLIF(TRIM(item_id::text), '')::UUID,
  ALTER COLUMN category_id TYPE UUID USING NULLIF(TRIM(category_id::text), '')::UUID;

ALTER TABLE "SalesInvoice"
  ALTER COLUMN customer_id TYPE UUID USING NULLIF(TRIM(customer_id::text), '')::UUID,
  ALTER COLUMN sales_order_id TYPE UUID USING NULLIF(TRIM(sales_order_id::text), '')::UUID;

ALTER TABLE "SalesReturn"
  ALTER COLUMN customer_id TYPE UUID USING NULLIF(TRIM(customer_id::text), '')::UUID,
  ALTER COLUMN sales_invoice_id TYPE UUID USING NULLIF(TRIM(sales_invoice_id::text), '')::UUID,
  ALTER COLUMN pos_sale_id TYPE UUID USING NULLIF(TRIM(pos_sale_id::text), '')::UUID;
...
ALTER TABLE "Quotation"
  ALTER COLUMN customer_id TYPE UUID USING NULLIF(TRIM(customer_id::text), '')::UUID,
  ALTER COLUMN converted_to_order_id TYPE UUID USING NULLIF(TRIM(converted_to_order_id::text), '')::UUID,
  ALTER COLUMN converted_to_invoice_id TYPE UUID USING NULLIF(TRIM(converted_to_invoice_id::text), '')::UUID;

ALTER TABLE "PurchaseInvoice"
  ALTER COLUMN vendor_id TYPE UUID USING NULLIF(TRIM(vendor_id::text), '')::UUID,
  ALTER COLUMN po_reference_id TYPE UUID USING NULLIF(TRIM(po_reference_id::text), '')::UUID;

ALTER TABLE "PurchaseReturn"
  ALTER COLUMN vendor_id TYPE UUID USING NULLIF(TRIM(vendor_id::text), '')::UUID,
  ALTER COLUMN purchase_invoice_id TYPE UUID USING NULLIF(TRIM(purchase_invoice_id::text), '')::UUID;
...
ALTER TABLE "ManufacturingOrder"
  ALTER COLUMN product_item_id TYPE UUID USING NULLIF(TRIM(product_item_id::text), '')::UUID;

ALTER TABLE "FinancialVoucher"
  ALTER COLUMN contact_id TYPE UUID USING NULLIF(TRIM(contact_id::text), '')::UUID;

ALTER TABLE "FixedAsset"
  ALTER COLUMN parent_asset_id TYPE UUID USING NULLIF(TRIM(parent_asset_id::text), '')::UUID,
  ALTER COLUMN asset_ledger_id TYPE UUID USING NULLIF(TRIM(asset_ledger_id::text), '')::UUID,
  ALTER COLUMN accumulated_dep_ledger_id TYPE UUID USING NULLIF(TRIM(accumulated_dep_ledger_id::text), '')::UUID,
  ALTER COLUMN dep_expense_ledger_id TYPE UUID USING NULLIF(TRIM(dep_expense_ledger_id::text), '')::UUID,
  ALTER COLUMN payment_account_id TYPE UUID USING NULLIF(TRIM(payment_account_id::text), '')::UUID;

ALTER TABLE "AssetComplianceSchedule"
  ALTER COLUMN asset_id TYPE UUID USING NULLIF(TRIM(asset_id::text), '')::UUID;

ALTER TABLE "DepreciationSchedule"
  ALTER COLUMN asset_id TYPE UUID USING NULLIF(TRIM(asset_id::text), '')::UUID;

ALTER TABLE "FinancialVoucherDeleteLog"
  ALTER COLUMN voucher_id TYPE UUID USING NULLIF(TRIM(voucher_id::text), '')::UUID;

ALTER TABLE "ItemDeleteLog"
  ALTER COLUMN item_id TYPE UUID USING NULLIF(TRIM(item_id::text), '')::UUID;

ALTER TABLE "PartnerDeleteLog"
  ALTER COLUMN partner_id TYPE UUID USING NULLIF(TRIM(partner_id::text), '')::UUID;
...
ALTER TABLE "GeneralLedgerLine" DROP CONSTRAINT IF EXISTS fk_gll_journal;
ALTER TABLE "GeneralLedgerLine" DROP CONSTRAINT IF EXISTS fk_gll_account;
ALTER TABLE "GeneralLedgerLine"
