-- Phase 2: Asynchronous Delta Patching & Audit

-- 1. Queue & Audit Tables
CREATE TABLE IF NOT EXISTS "pending_ledger_recalculations" (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  company_id UUID NOT NULL,
  fiscal_year_id UUID NOT NULL,
  module_type TEXT NOT NULL CHECK (module_type IN ('FINANCIAL', 'INVENTORY')),
  account_id UUID, -- For FINANCIAL
  item_id UUID,    -- For INVENTORY
  net_change_delta NUMERIC(15,2) NOT NULL,
  status TEXT DEFAULT 'pending' CHECK (status IN ('pending', 'processing', 'completed', 'failed')),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS "system_recalculation_logs" (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  company_id UUID NOT NULL,
  fiscal_year_id UUID NOT NULL,
...
BEGIN
  ...
  IF TG_OP = 'DELETE' THEN 
    ...
    -- For delete, the delta is 0 - OLD (assuming net is debit - credit)
    -- Actually, it depends on normal balance, but let's standardise on Net = Debit - Credit for the queue
...
DROP TRIGGER IF EXISTS flag_recalculation_fin ON "GeneralLedgerLine";
CREATE TRIGGER flag_recalculation_fin
AFTER INSERT OR UPDATE OR DELETE ON "GeneralLedgerLine"
FOR EACH ROW EXECUTE FUNCTION trigger_flag_recalculation_financial();
...
DECLARE
  ...
  v_item_id UUID;
BEGIN
  -- InventoryLedger has quantity_in and quantity_out
  IF TG_OP = 'DELETE' THEN 
    v_company_id := OLD.company_id; 
    v_item_id := OLD.item_id;
    -- For delete, we reverse what was inserted
  ...
  -- Find the transaction date (assuming it's on the line or header)
  -- For this example, assuming transaction_date is on the line
  v_transaction_date := COALESCE(NEW.transaction_date, OLD.transaction_date);

  -- Find the FY of this transaction
  SELECT * INTO v_fy FROM "FiscalYear" 
...
DROP TRIGGER IF EXISTS flag_recalculation_inv ON "InventoryLedger";
CREATE TRIGGER flag_recalculation_inv
AFTER INSERT OR UPDATE OR DELETE ON "InventoryLedger"
FOR EACH ROW EXECUTE FUNCTION trigger_flag_recalculation_inventory();


-- 3. Sweep Job (The Multi-Year Cascade)
  ...
  LOOP
    ...
    LOOP
      ...
      IF v_rec.module_type = 'FINANCIAL' THEN
        ...
        IF v_opening_journal_id IS NOT NULL THEN
          ...
          IF v_account_type IN ('Revenue', 'COGS', 'OPEX', 'Expense', 'Income') THEN
            ...
            -- Assuming GeneralLedgerLine has a unique constraint on (journal_id, account_id) for this to work natively.
            -- Without a strict unique constraint, we do a manual UPSERT:
            ...
            WHERE journal_id::uuid = v_opening_journal_id AND account_id = v_retained_earnings_id;
            
            IF NOT FOUND THEN
              INSERT INTO "GeneralLedgerLine" (company_id, journal_id, account_id, account_name, account_type, debit_amount, credit_amount)
              VALUES (v_rec.company_id, v_opening_journal_id::TEXT, v_retained_earnings_id, 'Retained Earnings', 'Equity', 
          ...
          ELSE
            ...
            SET 
              debit_amount  = GREATEST(0, (debit_amount - credit_amount) + v_rec.net_change_delta),
              credit_amount = GREATEST(0, 0 - ((debit_amount - credit_amount) + v_rec.net_change_delta))
            WHERE journal_id::uuid = v_opening_journal_id AND account_id = v_rec.account_id;
            ...
            IF NOT FOUND THEN
               ...
               BEGIN
                 ...
                 VALUES (v_rec.company_id, v_opening_journal_id::TEXT, v_rec.account_id, v_acc_details.account_code, v_acc_details.account_name, v_acc_details.account_type, 
                         GREATEST(0, v_rec.net_change_delta), GREATEST(0, 0 - v_rec.net_change_delta));
               END;
            END IF;
      ...
      ELSIF v_rec.module_type = 'INVENTORY' THEN
        ...
        -- In a real implementation, we would extract the line_items JSONB, find the item_id, update its quantity, and save it back.
        -- Due to JSONB complexity in SQL, a PL/PGSQL JSONB array replace logic goes here.
        -- Log Audit Trail
        INSERT INTO "system_recalculation_logs" (company_id, fiscal_year_id, log_message)
        VALUES (v_rec.company_id, v_subsequent_fys.id, 
...
-- Schedule the pg_cron job (Requires pg_cron extension, syntax depends on environment)
-- SELECT cron.schedule('process_recalculations_every_5_mins', '*/5 * * * *', 'SELECT process_pending_recalculations()');
