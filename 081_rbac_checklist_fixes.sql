-- 081_rbac_checklist_fixes.sql
-- Implements institutional finance compliance (Dual Control, Audit Security, Search Paths)

-- ============================================================================
-- 1. Restrict AuditLog read access to Tenant Admins
-- ============================================================================
...
-- ============================================================================
-- 2. Explicit Security Definer and Search Path for triggers and RPCs
-- ============================================================================

-- Re-create log_system_changes with strict security
...
REVOKE EXECUTE ON FUNCTION log_system_changes() FROM public, anon;
GRANT EXECUTE ON FUNCTION log_system_changes() TO authenticated, service_role;

-- Re-create check_user_operational_access_rpc with strict security
...
BEGIN
    ...
    FROM "UserCompany"
    WHERE user_id = p_user_id AND company_id::TEXT = p_company_id;
    ...
    ELSE
        ...
        FROM "UserCompany" WHERE user_id = p_user_id AND company_id::TEXT = p_company_id;
...
REVOKE EXECUTE ON FUNCTION check_user_operational_access_rpc(UUID, TEXT, TEXT, TEXT) FROM public, anon;
GRANT EXECUTE ON FUNCTION check_user_operational_access_rpc(UUID, TEXT, TEXT, TEXT) TO authenticated, service_role;

-- ============================================================================
-- 3. Restore GL Posting RBAC Enforcement with Maker-Checker Dual Control
-- ============================================================================
...
BEGIN
    ...
    IF auth.uid() IS NOT NULL THEN
        -- 1. Enforce 'approve' / 'reverse' permission
