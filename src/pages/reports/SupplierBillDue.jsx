...
export default function SupplierBillDue() {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const apId = selectedBill.payable_account_id || settings.gl_accounts_payable_id;
      if (!apId) throw new Error("AP Account not mapped in settings or invoice");

      const targetAccount = cashAccounts.find(a => a.id === payForm.account_id);

      // 1. Create Financial Voucher
      const voucher_number = `PV-${new Date().getFullYear()}-${Date.now().toString().slice(-4)}`;
      const allocations = [{
      ...
      const voucher = await sajilo.entities.FinancialVoucher.create({
        ...
        voucher_number,
        contact_name: selectedBill.vendor_name,
        payment_mode: 'Bank Transfer',
        reference_no: payForm.reference,
        narration: `Payment against Invoice ${selectedBill.invoice_number}`,
      ...
      // 2. Post to GL
      const lines = [
        { account_id: payForm.account_id, debit: 0, credit: Number(payForm.amount), narration: 'Amount Paid' },
        { account_id: apId, debit: Number(payForm.amount), credit: 0, narration: `Against ${selectedBill.invoice_number}` }
      ];
      ...
      await supabase.from('PurchaseInvoice').update({ paid_amount: newPaid, payment_status: newStatus }).eq('id', selectedBill.id);

      toast.success('Payment Made & Allocated');
  ...
  const viewAllocations = async (bill) => {
    ...
    // For simplicity since JSONB querying is complex, we just fetch Payments for this supplier and filter locally
    const { data: payments } = await supabase.from('FinancialVoucher').select('voucher_number, voucher_date, total_amount, bill_allocations').eq('voucher_type', 'Payment').eq('status', 'Posted').eq('contact_name', bill.vendor_name);
    const related = [];
    (payments || []).forEach(r => {
  ...
  const openReconModal = async () => {
    ...
    const { data: pData } = await supabase.from('BusinessPartner').select('id, name').eq('is_vendor', true);
    setPartners(pData || []);
    setContactFilter('');
  ...
  const columns = [
    ...
    { key: 'actions', label: 'Action', render: (_, row) => (
      <div className="flex gap-2">
        <Button size="sm" onClick={() => openPayModal(row)}><DollarSign className="w-4 h-4 mr-1"/> Make Payment</Button>
  ...
  return (
    <div>
      ...
      <Dialog open={payModalOpen} onOpenChange={setPayModalOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Make Payment for {selectedBill?.invoice_number}</DialogTitle></DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-4 mt-2">
            <div className="border border-border p-3 rounded">
              ...
              <select className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring mt-1" 
                ...
                {data.filter(d => d.due > 0 && (!contactFilter || d.vendor_name === contactFilter)).map(d => (
