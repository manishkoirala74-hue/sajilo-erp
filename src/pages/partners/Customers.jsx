...
const emptyForm = {
  ...
  treated_as_vendor: false, profile_picture_url: '',
...
export default function Customers() {
  ...
  const enrichWithAccountCodes = async (partners, settingsData) => {
    ...
    for (const p of partners) {
      if (p.receivable_account_id) {
        const acc = byId[p.receivable_account_id];
        if (acc && acc.ledger_type === 'Sub Ledger') {
          enriched.push({ ...p, receivable_account_code: acc.account_code, receivable_account_name: acc.account_name });
        } else if (acc && acc.ledger_type === 'Group Ledger' && settingsData?.gl_customer_ledger_group_id) {
          try {
            const ledger = await createPartnerLedger({ partnerName: p.name, parentGroupId: settingsData.gl_customer_ledger_group_id, accountType: 'Asset', normalBalance: 'Debit', accountSubtype: 'Current Asset' });
            await sajilo.entities.BusinessPartner.update(p.id, { receivable_account_id: ledger.id, receivable_account_name: ledger.account_name, receivable_account_code: ledger.account_code });
            enriched.push({ ...p, receivable_account_id: ledger.id, receivable_account_code: ledger.account_code, receivable_account_name: ledger.account_name });
          } catch { enriched.push(p); }
        } else { enriched.push(p); }
      } else if (settingsData?.gl_customer_ledger_group_id) {
        try {
          const ledger = await createPartnerLedger({ partnerName: p.name, parentGroupId: settingsData.gl_customer_ledger_group_id, accountType: 'Asset', normalBalance: 'Debit', accountSubtype: 'Current Asset' });
          await sajilo.entities.BusinessPartner.update(p.id, { receivable_account_id: ledger.id, receivable_account_name: ledger.account_name, receivable_account_code: ledger.account_code });
          enriched.push({ ...p, receivable_account_id: ledger.id, receivable_account_code: ledger.account_code, receivable_account_name: ledger.account_name });
  ...
  useEffect(() => {
    Promise.all([
      sajilo.entities.BusinessPartner.filter({ is_customer: true }, '-created_date'),
  ...
  const handleDelete = async (partner) => {
    ...
    if (blockedReason) {
      toast.error(`Cannot delete: Customer "${partner.name}" has ${blockedReason}.`);
      return;
    }
  ...
  return (
    <div>
      <PageHeader
        title="Customers"
        subtitle="Manage customer accounts and trade receivable ledgers"
        action={openNew}
      ...
      <div className="bg-white border border-border rounded-xl overflow-hidden">
            ...
            {loading ? (
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center overflow-hidden">
                      {row.profile_picture_url ? (
                        ...
                        row.partner_type === 'Company' ? <Building2 className="w-4 h-4 text-blue-600" /> : <User className="w-4 h-4 text-blue-600" />
                      )}
                    </div>
                    <div>
                ...
                <td className="px-4 py-2.5">
                  {row.treated_as_vendor ? <span className="px-2 py-0.5 rounded-full text-xs bg-purple-100 text-purple-700 font-medium">Also Vendor</span> : null}
                </td>
                <td className="px-4 py-2.5">
                  {row.receivable_account_name
                    ? <div><span className="text-xs font-mono font-semibold text-emerald-700">{row.receivable_account_code || ''}</span>{row.receivable_account_code && <span className="text-xs text-muted-foreground mx-1">—</span>}<span className="text-xs text-emerald-700">{row.receivable_account_name}</span></div>
        ...
        <div className="px-4 py-2 border-t border-border text-xs text-muted-foreground">
          {filteredPartners.length} customer{filteredPartners.length !== 1 ? 's' : ''}
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            ...
            <div className="col-span-2">
              ...
              <Input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Himalayan Trading Co." className="mt-1" />
            </div>
            <div>
            ...
            <div className="col-span-2 bg-muted/50 rounded-lg p-4 space-y-3">
              <div className="flex items-center gap-3">
                <Switch checked={!!form.treated_as_vendor} onCheckedChange={v => setForm({ ...form, treated_as_vendor: v, is_vendor: v })} />
                <div>
                  <Label className="cursor-pointer">Treated as Vendor</Label>
                  <p className="text-xs text-muted-foreground">Creates a twin AP sub-ledger under Supplier Group and activates this partner in Purchase module dropdowns.</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
            ...
            {editing?.receivable_account_name && (
