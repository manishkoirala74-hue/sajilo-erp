...
export default function Customers() {
  ...
  const enrichWithAccountCodes = async (partners, settingsData) => {
    ...
    for (const p of partners) {
      ...
      } else if (p.payable_account_id) {
        // Re-use AP ledger for AR if it exists
        await sajilo.entities.BusinessPartner.update(p.id, {
          receivable_account_id: p.payable_account_id,
          receivable_account_name: p.payable_account_name,
          receivable_account_code: p.payable_account_code
        });
        enriched.push({ ...p, receivable_account_id: p.payable_account_id, receivable_account_code: p.payable_account_code, receivable_account_name: p.payable_account_name });
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-500/20 flex items-center justify-center overflow-hidden">
                      {row.profile_picture_url ? (
                        ...
                        row.partner_type === 'Company' ? <Building2 className="w-4 h-4 text-blue-600 dark:text-blue-400" /> : <User className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                      )}
                    </div>
                    <div>
            ...
            <div className="col-span-2 bg-muted/50 rounded-lg p-4 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Uses the same Customer AR ledger for AP transactions and activates this partner in Purchase dropdowns.</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
