 ...
 * (stock is still updated — GL is best-effort until accounts are configured in Settings).
 */
...
function warnMissingAccount(name) {
  toast.warning(`GL Posting skipped: "${name}" account not configured in Settings → GL Accounts.`, { duration: 6000 });
}
 ...
 *   DR COGS               (qty × WAC, physical items only)
 *   CR Inventory Asset    (qty × WAC, physical items only)
 *
...
export async function postPOSSale(sale, itemsMap, settings, isReversal = false) {
  ...
  const journalId = await createJournal({
    date: sale.sale_date,
    description: `POS Sale ${sale.sale_number}${isReversal ? ' — VOIDED' : ''}`,
    module: 'Sales',
    sourceId: sale.id,
    sourceType: 'POSSale',
    lines,
...
export async function postSalesInvoice(invoice, itemsMap, settings, isReversal = false) {
  ...
  const journalId = await createJournal({
    ...
    description: `Sales Invoice ${invoice.invoice_number}${isReversal ? ' — CANCELLED' : ''}`,
    module: 'Sales',
    sourceId: invoice.id,
 ...
 *   new_total_value = total_asset_value + (received_qty × unit_price)
 ...
 *   DR VAT Receivable/Input  (if VAT applicable — treated as input tax recoverable)
...
export async function postPurchaseInvoice(invoice, itemsMap, settings, isReversal = false) {
  ...
  if (invoice.vat_amount > 0 && s.gl_vat_payable_id) {
    // Input VAT — DR VAT Payable (reduces liability / treated as input credit)
  ...
  const journalId = await createJournal({
    date: invoice.invoice_date,
    description: `Purchase Invoice ${invoice.invoice_number}${isReversal ? ' — CANCELLED' : ''}`,
    module: 'Purchase',
    sourceId: invoice.id,
...
/**
 * On Post:
 *   DR Sales Returns & Allowances  (subtotal — contra-revenue)
 *   DR VAT Payable                 (vat reversal)
 *   CR Cash / Bank                 (refund paid out — asset decreases)
...
/**
 * On Post:
 *   DR Accounts Payable      (reduces liability — vendor owes us)
...
export async function postStockAdjustment(adj, itemsMap, settings) {
  ...
  return createJournal({
    ...
    description: `Stock Adjustment ${adj.adjustment_number} — ${adj.reason}`,
    module: 'Stock',
    sourceId: adj.id,
 ...
 *   CR Stock Variance    (balancing — represents opening equity injection)
 *
...
export async function postOpeningStock(items, settings, date, offsetAccount = null, inventoryAccount = null) {
  ...
  // Build a name→account map to resolve account IDs from names when IDs are missing
  ...
  return createJournal({
    date: entryDate,
    description: 'Opening Stock — Item Import',
    module: 'Stock',
    sourceId: 'import',
...
// ─── 8. ITEM DELETION — Inventory Write-Off ──────────────────────────────────
/**
 ...
 *   CR Inventory Asset   (qty × current_unit_cost)  — removes asset
...
export async function postItemDeletionWriteOff(items, settings) {
  ...
  return createJournal({
    date: today,
    description: `Inventory Write-Off — ${items.length} item(s) deleted`,
    module: 'Stock',
 ...
 *
 *   DR Asset Cost Ledger      (gross_purchase_value)   — asset.asset_ledger_id
 *   CR Accounts Payable / Cash (gross_purchase_value)  — s.gl_accounts_payable_id
 *
...
export async function postAssetPurchase(asset, settings, isReversal = false, preloadedAccounts = null, creditAccount = null) {
  ...
  // Asset cost debit account — MUST be per-asset or fail
  const assetAccId   = asset.asset_ledger_id;
  const assetAccName = asset.asset_ledger_name || asset.asset_name + ' (Cost)';
  if (!assetAccId) {
    warnMissingAccount(`Asset Cost Ledger for "${asset.asset_name}" — set it in the asset form`);
    return null;
  }

  // Credit: use explicit creditAccount if provided, otherwise fall back to AP → Cash from settings
  let apId, apName;
  if (creditAccount?.id) {
  ...
  if (!apId) {
    warnMissingAccount(
      '"Accounts Payable" account not configured in Settings → GL Accounts. ' +
  ...
  return createJournal({
    ...
    description: `Fixed Asset ${isReversal ? 'Write-Off' : 'Purchase'} — ${asset.asset_name} (${asset.asset_code || ''})`,
    module: 'Assets',
    sourceId: asset.id,
...
// ─── 10. ASSET DEPRECIATION (per-asset ledger wiring) ────────────────────────
/**
...
export async function postAssetDepreciation(asset, depAmount, periodLabel, settings) {
  ...
  // Resolve expense account: per-asset → category fallback
  ...
  // Resolve credit account: per-asset → category fallback
  ...
  const crAccName = asset.accumulated_dep_ledger_name || (
    ['Machinery', 'IT Equipment'].includes(asset.category)
      ? (s.dep_accumulated_machinery_account_name || 'Accum. Dep. — Machinery')
      : asset.category === 'Vehicles'
        ? (s.dep_accumulated_vehicle_account_name || 'Accum. Dep. — Vehicles')
        : (s.dep_accumulated_office_account_name || 'Accum. Dep. — Office')
  );
  ...
  const lines = [
    { account_id: expAccId, account_name: expAccName, debit_amount: amt,  credit_amount: 0,   description: `Dep. expense — ${asset.asset_name} (${periodLabel})` },
    { account_id: crAccId,  account_name: crAccName,  debit_amount: 0,    credit_amount: amt,  description: `Accum. dep. — ${asset.asset_name} (${periodLabel})` },
  ];

  return createJournal({
    date: new Date().toISOString().slice(0, 10),
    description: `Depreciation — ${asset.asset_name} (${periodLabel})`,
    module: 'Assets',
    sourceId: asset.id,
    sourceType: 'FixedAsset',
    lines,
 ...
 *   asset                    — FixedAsset record (with ledger IDs populated)
 *   settings                 — CompanySettings record
 *   proceeds                 — cash/bank proceeds received (0 if none)
 *   proceedsPaymentMethod    — 'Cash' | 'Bank' (default: 'Cash')
 *   disposalDate             — ISO date string
 *   manual_disposal_ledger_id — (optional) FK override for the Gain/Loss line
 *
 * Journal structure:
 *   DR  Accumulated Dep. Ledger     (accumulated_depreciation)         — removes contra-asset
 *   DR  Cash / Bank                 (proceeds, if any)                 — realized inflow
 *   DR  Loss on Disposal            (if NBV > proceeds, i.e. loss)     — expense
 *   CR  Asset Cost Ledger           (gross_purchase_value)             — removes asset
 *   CR  Gain on Disposal            (if proceeds > NBV, i.e. gain)     — income
 *
...
export async function postAssetDisposal({
  ...
  // ── Validate mandatory asset ledgers ──────────────────────────────────────
  ...
  // ── Gain/Loss account resolution (3-tier) ────────────────────────────────
  let glAccId   = null;
  let glAccName = null;

  if (manual_disposal_ledger_id) {
    // Tier 1: explicit manual override — trust the caller, resolve name from CoA
    ...
    if (hit) {
      ...
      if (!glAccId) {
        warnMissingAccount(
          `Gain/Loss on Disposal account — create "${targetName}" in Chart of Accounts or set a manual_disposal_ledger_id`
        );
        return null;
      }
    }
  ...
  if (accumDep > 0) {
    lines.push({
      ...
      description: `Disposal: remove accum. dep. — ${asset.asset_name}`,
    });
  }
  ...
  lines.push({
    ...
    description: `Disposal: remove asset cost — ${asset.asset_name}`,
  });

  // Gain (CR) or Loss (DR) — uses the resolved glAccId from the 3-tier logic above
  ...
  return createJournal({
    date,
    description: `Asset Disposal — ${asset.asset_name} (${asset.asset_code || ''}) | NBV: ${nbv} | Proceeds: ${proc} | ${gainOrLoss >= 0 ? 'Gain' : 'Loss'}: ${Math.abs(gainOrLoss)}`,
    module: 'Assets',
    sourceId: asset.id,
    sourceType: 'FixedAsset',
    lines,
...
// ─── Utility: load all items as a map {id → item} ────────────────────────────
