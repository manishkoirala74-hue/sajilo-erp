    ...
    <PopoverAnchor asChild>
      <div className={cn("relative w-full", className)}>
        <CommandPrimitive.Input
          ...
          className={cn(
            "flex h-11 sm:h-9 w-full rounded-md border border-border bg-background px-3 py-2 pr-8 text-base sm:text-sm shadow-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary disabled:cursor-not-allowed disabled:opacity-50",
            disabled && "cursor-not-allowed opacity-50 bg-muted/50"
          )}
          {...props}
        />
        <ChevronDown 
          className="absolute right-3 top-3.5 sm:top-2.5 h-4 w-4 opacity-50 cursor-pointer hover:opacity-100" 
          onClick={(e) => {
            e.stopPropagation();
